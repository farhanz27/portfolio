...
export default function Footer() {
  return (
    <footer className="border-t border-sky-100/50 bg-white/35 backdrop-blur-sm text-slate-500">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
        <div className="flex flex-col md:flex-row items-start justify-between gap-8 mb-10">
          {/* Logo + tagline */}
          <div>
            <div className="flex items-center gap-2 mb-3">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-sky-400 to-teal-500 flex items-center justify-center shadow-lg shadow-sky-400/15">
            ...
            <div className="flex gap-3 mb-6">
              {socials.map(({ icon: Icon, href, label }) => (
                <motion.a
                  ...
                  whileTap={{ scale: 0.9 }}
                  className="w-9 h-9 rounded-xl glass border border-white/70 flex items-center justify-center text-slate-500 hover:text-slate-900 hover:border-sky-200 hover:shadow-sm transition-all"
                >
        ...
        {/* Divider */}
        <div className="border-t border-sky-100/40 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-slate-400">
            © {new Date().getFullYear()} Mohammad Farhan. All rights reserved.
          </p>
          <p className="text-xs text-slate-400 flex items-center gap-1.5">
            Built with <Heart size={11} className="text-rose-400" /> using Next.js, Tailwind & Framer Motion
          </p>
        </div>
      </div>
    </footer>
